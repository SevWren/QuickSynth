...
try {
    if (document.getElementById(POPUP_ID)) {
        ...
        // return; // In a normal script, you might not return from the global scope
    }
    ...
    categoryBoxes.forEach(categoryBox => {
        ...
        articleLinks.forEach(link => {
            ...
            if (title && url) {
                ...
                uniqueCategories.add(categoryTitle);
            }
        });
    });
    ...
    closeButton.innerHTML = "×"; // Corrected from '×' if it was causing issues, × is safer.
    ...
    const categoriesHeaderH4 = document.createElement("h4");
    applyStyles(categoriesHeaderH4, {
    ...
    categoriesHeaderH4.textContent = "Filter by Category";
    categoriesSection.appendChild(categoriesHeaderH4);
        ...
        categoryCheckbox.onchange = (e) => {
            ...
            articlesData.forEach(article => {
                if (article.category === categoryName) {
                    article.include = isChecked;
                    const articleCheckbox = document.getElementById(`checkbox-for-${article.id}`);
                    const articleLabel = document.querySelector(`label[for='checkbox-for-${article.id}']`);
                    if (articleCheckbox) {
                        articleCheckbox.checked = isChecked;
                        articleCheckbox.disabled = !isChecked;
                        if (articleLabel) {
                            articleLabel.style.opacity = isChecked ? "1" : "0.6";
                        }
                    }
                }
            });
    ...
    const articlesHeaderH4 = document.createElement("h4");
    applyStyles(articlesHeaderH4, {
        fontSize: STYLES.fontSizeBase,
        fontWeight: "600",
        color: STYLES.colors.textDim,
    ...
    articlesHeaderH4.textContent = "Articles";
    contentArea.appendChild(articlesHeaderH4);
    ...
    copyButton.onmouseover = () => copyButton.style.backgroundColor = STYLES.colors.selectedBorder; // Using a different color for hover
